...
describe("agent auth middleware", () => {
  ...
  it.each([
    ...
    ["malformed bearer token", "Bearernot-a-token", "Malformed bearer authorization"],
  ...
  it.each([
    ["an agent record in another company", "active", true, "missing or belongs to another company"],
    ["a terminated agent", "terminated", false, "Agent is terminated"],
    ["a pending agent", "pending_approval", false, "Agent is pending approval"],
  ])("rejects a static key for %s", async (_label, status, wrongCompany, error) => {
    const keyCompanyId = randomUUID();
    const agentId = randomUUID();
    const token = "pcp_test_denied_agent_key";
    const { db } = createDbState({
      agent: {
        id: agentId,
        companyId: wrongCompany ? randomUUID() : keyCompanyId,
        status,
      },
      agentKey: {
        id: randomUUID(),
