...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const hasMalformedBearerCredentials =
      /^bearer/i.test(authHeader ?? "") && !/^bearer(?:\s|$)/i.test(authHeader ?? "");
    if (hasMalformedBearerCredentials) {
      next(
        unauthorized("Malformed bearer authorization; separate the Bearer scheme and token with whitespace"),
      );
      return;
    }
